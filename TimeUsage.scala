...
object TimeUsage {
  ...
  */
  ...
  def dfSchema(columnNames: List[String]): StructType = {
    // { YOUR CODE HERE }
    val colHead = columnNames.head
    val colTail = columnNames.tail
    val strucFieldHead = StructField(colHead,StringType,false)
    val strucFieldTails = colTail.map(cn=>StructField(cn,DoubleType,false))
    
  StructType(strucFieldHead :: strucFieldTails)
  }
  ...
  def classifiedColumns(columnNames: List[String]): (List[Column], List[Column], List[Column]) ={
    ...
    val o = List("t02", "t04", "t06", "t07", "t08", "t09", "t10", "t12", "t13", "t14", "t15", "t16","t18")

    val primaryNeeds = columnNames.filter(cn=>p.exists(s=>cn.startsWith(s))).map(c=>col(c))
    val work = columnNames.filter(cn=>w.exists(s=>cn.startsWith(s))).map(c=>col(c))
    val other = columnNames.filter(cn=> !cn.startsWith("t1801")&&
                                        !cn.startsWith("t1803")&&
                                        !cn.startsWith("t1805")&&
                                        o.exists(s=>cn.startsWith(s))).map(c=>col(c))
  ...
  def timeUsageSummary( primaryNeedsColumns : List[Column]
    ...
    val primaryNeedsProjection: Column = primaryNeedsColumns.reduce(_+_)
                                                            .divide(60.0)
                                                            .as("primaryNeedsProjection")//??? // { YOUR CODE HERE }
      
    val workProjection: Column = workColumns.reduce(_+_)
                                            .divide(60.0)
                                            .as("workProjection")//??? // { YOUR CODE HERE }
      
    val otherProjection: Column = otherColumns.reduce(_+_)
                                              .divide(60.0)
                                              .as("otherProjection")//??? // { YOUR CODE HERE }
  ...
  def timeUsageGrouped(summed: DataFrame): DataFrame = {
    ...
    // val dfWorking = summed.groupBy("working")

    // val dfsex = summed.groupBy("sex")
                      
    // val dfage = summed.groupBy("age")
    
    // val dfprimaryNeeds =  dfWorking.groupBy("primaryNeeds")
    //                             .round(avg(""))// { YOUR CODE HERE }
    ...
    ds.agg(round(avg("primaryNeedsprojection"),1).as("primaryNeeds"),
